...
- `CompaniesPage` — home (`/`); lists ecological-expertise consultants aggregated from `public/new-companies.json`, sortable/searchable/paginated via `src/utils/dataUtils.js`.
...
- static files under `public/` (`new-companies.json`, PDFs in `public/arizalar/`, `234-qaror.pdf`) fetched at runtime with `fetch()`, or
...
`src/utils/dataUtils.js` loads and caches `public/new-companies.json` once (`_cache`) and derives everything (company aggregates, dashboard chart series, loyihachi rating tiers) from that single flat record array client-side — there is no server-side aggregation. Unlike the old `companies.json`, `new-companies.json` is a bare top-level array (not wrapped in a `{ companies: [...] }` object). Records use Cyrillic Uzbek fields transliterated to Latin via the `toLatin()` map in the same file; reuse that helper rather than adding a new transliteration table.
