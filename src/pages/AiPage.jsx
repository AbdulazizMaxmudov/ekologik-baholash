...
import { Upload, FileText, X, Sparkles, XCircle, AlertTriangle, ChevronDown, ChevronUp, RotateCcw } from 'lucide-react'
...
const STEPS = ["PDF o'qilmoqda...", 'Loyiha yuborilmoqda...', 'Hujjat tahlil qilinmoqda...', "Mezonlar bo'yicha baholanmoqda...", 'Natija tayyorlanmoqda...']
...
function LoadingState({ elapsed }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16">
      ...
      <p className="text-base font-semibold text-slate-800 mb-1">{STEPS[stepIdx]}</p>
      <p className="text-sm text-slate-400 mb-6">{elapsed} soniya o'tdi</p>
      <div className="flex gap-1.5 mb-8">
        {STEPS.map((_,i) => (
          <div key={i} className={`h-1.5 rounded-full transition-all duration-500 ${i<=stepIdx?'bg-emerald-500 w-8':'bg-slate-200 w-4'}`} />
        ))}
      </div>
...
export default function AiPage() {
  ...
  const handleEvaluate = async () => {
    ...
    try {
      const res = await evaluateProject(file)
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      ...
      {status === 'done' && result ? <ResultView result={result} onReset={handleReset} /> : (
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          {status === 'loading' ? <LoadingState elapsed={elapsed} /> : (
            <div className="space-y-5">
              ...
              <button onClick={handleEvaluate} disabled={!file}
